...
namespace Snoop {
    public class VisualTree3DView : Viewport3D {
        static readonly Pen OutlinePen = new Pen(new SolidColorBrush(Color.FromArgb(128, 255, 0, 0)), 2);

        readonly bool drawOutlines = false;
        readonly bool includeEmptyVisuals = false;
        readonly TrackballBehavior trackballBehavior;
        readonly ScaleTransform3D zScaleTransform;
        ...
        public double ZScale {
            get { return zScaleTransform.ScaleZ; }
            set { zScaleTransform.ScaleZ = value; }
        }

        public void Reset() {
            trackballBehavior.Reset();
            ZScale = 1;
        }

        Model3D ConvertVisualToModel3D(Visual visual, ref double z) {
            Model3D model = null;
            ...
            if (childrenCount > 0) {
                ...
                for (var i = 0; i < childrenCount; i++) {
                    var childVisual = CommonTreeHelper.GetChild(visual, i) as Visual;
                    if (childVisual != null) {
            ...
            if (model != null) {
                ...
                if (!matrix.IsIdentity) {
                    var matrix3D = new Matrix3D(matrix.M11, matrix.M12, 0, 0, matrix.M21, matrix.M22, 0, 0, 0, 0, 1, 0,
                        matrix.OffsetX, matrix.OffsetY, 0, 1);
