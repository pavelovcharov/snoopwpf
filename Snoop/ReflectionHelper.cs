    ...
    public class ReflectionHelper {
        ...
        Action<object, object> CreateSetter(PropertyInfo info) {
            ...
            return
                (Action<object, object>)
                    CreateMethodHandlerImpl(info.GetSetMethod(true), null, typeof(Action<object, object>), true, null,
                        typeof(object));
        }

        static object CreateMethodHandlerImpl(object instance, string methodName, BindingFlags bindingFlags,
            Type instanceType, Type delegateType, bool castParameters, Type forcedResultType = null,
            Type forcedThisArgType = null, int? parametersCount = null, Type[] typeParameters = null) {
            MethodInfo mi = null;
            if (instance != null)
            ...
            return CreateMethodHandlerImpl(mi, instanceType, delegateType, castParameters, forcedResultType,
                forcedThisArgType);
        }

        static MethodInfo GetMethod(Type type, string methodName, BindingFlags bindingFlags,
            int? parametersCount = null, Type[] typeParameters = null) {
        ...
        static void CastClass(ILGenerator generator, Type sourceType, Type targetType) {
            ...
            if (Equals(sourceType, targetType))
                return;
            var oneIsVoid = Equals(typeof(void), sourceType) || Equals(typeof(void), targetType);
        ...
        static object CreateMethodHandlerImpl(MethodInfo mi, Type instanceType, Type delegateType,
            bool castParameters, Type forcedResultType = null, Type forcedThisArgType = null) {
            ...
            if (Equals(typeof(void), mi.ReturnType) && forcedResultType != null && forcedResultType != typeof(void))
                throw new ArgumentException(
                    "The forcedResultType argument should be either null or typeof(void) for the method which doesn't return a value");
        ...
        static Delegate CreateFieldGetterOrSetter<TElement, TField>(bool isGetter, Type delegateType,
            Type declaringType, string fieldName, BindingFlags bFlags) {
            var fieldInfo = declaringType.GetField(fieldName, bFlags);
            var isStatic = fieldInfo.IsStatic;
            DynamicMethod dm;
            if (isGetter)
                dm = new DynamicMethod(string.Empty, typeof(object), new[] {typeof(object)}, true);
            else
                dm = new DynamicMethod(string.Empty, typeof(void), new[] {typeof(object), typeof(object)}, true);
            var ig = dm.GetILGenerator();
        ...
        public static Delegate CreateInstanceMethodHandler(object instance, string methodName, BindingFlags bindingFlags,
            Type instanceType, bool castParameters = false, Type forcedResultType = null, Type forcedThisArgType = null,
            int? parametersCount = null, Type[] typeParameters = null) {
            return
                (Delegate)
                    CreateMethodHandlerImpl(instance, methodName, bindingFlags, instanceType, null, castParameters,
                        forcedResultType, forcedThisArgType, parametersCount, typeParameters);
        }

        public static TDelegate CreateInstanceMethodHandler<TInstance, TDelegate>(TInstance entity, string methodName,
            BindingFlags bindingFlags, bool castParameters = false, Type forcedResultType = null,
            Type forcedThisArgType = null, int? parametersCount = null, Type[] typeParameters = null)
            where TInstance : class {
            return
                (TDelegate)
                    CreateMethodHandlerImpl(entity, methodName, bindingFlags, typeof(TInstance), typeof(TDelegate),
                        castParameters, forcedResultType, forcedThisArgType, parametersCount, typeParameters);
        }

        public static TDelegate CreateInstanceMethodHandler<TDelegate>(object instance, string methodName,
            BindingFlags bindingFlags, Type instanceType, bool castParameters = false, Type forcedResultType = null,
            Type forcedThisArgType = null, int? parametersCount = null, Type[] typeParameters = null) {
            return
                (TDelegate)
        ...
        public T GetStaticMethodHandler<T>(Type entityType, string methodName, BindingFlags bindingFlags,
            bool castParameters = false, Type forcedResultType = null, Type forcedThisArgType = null) where T : class {
            object method;
            var key = new HelperKey(entityType, methodName, typeof(T));
            if (!InvokeInfo.TryGetValue(key, out method)) {
                method = CreateMethodHandlerImpl(null, methodName, bindingFlags, entityType, typeof(T), castParameters,
                    forcedResultType, forcedThisArgType);
                InvokeInfo[key] = method;
            }
            return (T) method;
        }

        public T GetInstanceMethodHandler<TInstance, T>(TInstance entity, string methodName, BindingFlags bindingFlags,
            bool castParameters = false, Type forcedResultType = null, Type forcedThisArgType = null,
            int? parametersCount = null) where TInstance : class {
            object method;
            var key = new HelperKey(entity.GetType(), methodName, typeof(T), castParameters, forcedResultType,
                forcedThisArgType, parametersCount);
            if (!InvokeInfo.TryGetValue(key, out method)) {
                method = CreateInstanceMethodHandler<TInstance, T>(entity, methodName, bindingFlags, castParameters,
                    forcedResultType, forcedThisArgType, parametersCount);
                InvokeInfo[key] = method;
            }
            return (T) method;
        }
        ...
        struct HelperKey {
            public bool Equals(HelperKey other) {
                return type == other.type && string.Equals(handlerName, other.handlerName) &&
                       Equals(handlerType, other.handlerType) && castParameters.Equals(other.castParameters) &&
                       forcedResultType == other.forcedResultType && forcedThisArgType == other.forcedThisArgType &&
            ...
            public HelperKey(Type type, string handlerName, Type handlerType, bool castParameters = false,
                Type forcesResultType = null, Type forcedThisArgType = null, int? parametersCount = null) {
                this.type = type;
